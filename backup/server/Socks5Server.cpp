...
Ndn_socket* Socks5Server::ndn_accept_local() {
	char buff[1000] ;
	memset(buff,0,1000) ;
	...
	Ndn_socket *nndn_socket = new Ndn_socket();
	nndn_socket->listen(new_prefix.data()) ;
	nndn_socket->set_daddr(buff) ;
	//sleep(10) ; 
	nndn_socket->write(new_prefix.data() , new_prefix.size()) ;
	//mndn_socket.
	return nndn_socket ;
}
...
void Socks5Server::ndn_transData(Ndn_socket &ndn_socket , int remote_sockfd){
	printf("Data transmission is beginning.\n");
...
void Socks5Server::ndn_forever(){
	while(true){
		Ndn_socket* ndn_socketp = this->ndn_accept_local() ;
		if(this->ndn_negotShake(*ndn_socketp)>0){
			cout<<"negotShake error"<<endl;
			ndn_socketp->close() ;
			delete(ndn_socketp) ;
			continue;
		} 
		pair<int,int> pairval= ndn_connShake(*ndn_socketp) ;
		if(pairval.second>0){
			cout<<"connShake error"<<endl;
			ndn_socketp->close() ;
			delete(ndn_socketp) ;
		...
		int remote_sockfd = pairval.first;
		ndn_transData(*ndn_socketp , remote_sockfd) ;
		// close the fds in main process
		ndn_socketp->close() ;
		delete(ndn_socketp) ;
		close(remote_sockfd);
